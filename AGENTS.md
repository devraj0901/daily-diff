...
2. Inspect the candidate JSON injected by `scripts/collect_candidates.py`. Reddit is **not** collected by that script; use Camofox MCP directly as the sole Reddit adapter.
...
6. Select at most **5 items** across all sources; prefer application-oriented tools, project write-ups, workflows, self-hosted systems, practical AI engineering, and discussions that generate build ideas. Use Camofox MCP and GitHub Trending as discovery sources, then read the linked primary project/article before accepting it. Keep excellent deep technical work eligible, but do not let academic papers, C++, compiler internals, or low-level performance pieces dominate the edition unless they have a clear practical payoff. A healthy mix is usually 2–3 practical/application items, 1 strong technical essay, and 0–2 wildcard ideas. Exclude press releases, generic listicles, shallow opinion, duplicate coverage, and marketing copy. Karakeep items are eligible, not automatic inclusions.
   - Enforce source diversity: no more than 2 GitHub Trending items in an edition of 5; when credible candidates exist, include at least 2 non-GitHub items and target 3 distinct source types. If feeds fail, use Camofox Reddit, direct HN/Algolia scanning, and Karakeep MCP before accepting more GitHub items. Publish fewer than 5 rather than filling the edition with one source.
